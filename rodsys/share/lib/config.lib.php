...
/*
		if ($config["cssInsertRename"]==1)
			...
			rename("css/".$cssName.".css","admin/css/".$cssName.".css_".date("Ymdhms"));
			}
*/
		}
...
function lngLoader($config,$fileName,$fileType,$filePath,$loadError)
	...
	if ($fileContent["returnCode"]!=1)
		...
		if (!empty($core_configsID))
			{
			$searchSQL = "SELECT * FROM core_language WHERE active=1 AND systemType='admin' AND core_configsID=10";
			...
			$searchSQL = "SELECT * FROM core_language WHERE active=1 AND systemType='admin' AND core_configsID='".$core_configsID."'";
			foreach ($config["dbConnect"]->query($searchSQL) as $configLngROW) 
				{
				if (substr($configLngROW["fieldValue"],0,2)!="//")
					{
...
function modulOperation($config)
	...
	if (isset($_POST["modulOperation"]) && !empty($_POST["modulOperation"]))
		...
		switch ($modulOperationCFG[0])
			...
			case "home":
				{
				header("Location: ".$config["configShare"]["baseModul"]);
				break;
				}
			case "list":
				{
...
*/
			...
			case "simple":
				{
//echo "hurra<br>";
//echo  $modulOperationCFG[1].'<br>';
//die();
				if ($modulOperationCFG[1] == "home" || $modulOperationCFG[1] == "logout") {
					header("Location: ".$config["configShare"]["baseModul"]);
				}
